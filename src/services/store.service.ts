...
export class StoreService {
	...
	public constructor() {
		openIDBUtilities(STORE_CONFIG)
			.then(stores => {
				this.stores = stores;
				this.user = <UserStore> {...this.user, ...this.initializeCachedValues(STORE_NAMES.user)};
				this.tokens = <TokenStore> {...this.tokens, ...this.initializeCachedValues(STORE_NAMES.tokens)};
			});
	}

	public async setStore(storeName: 'user' | 'tokens', props: {}): Promise<void> {
		Object.keys(props).forEach(async (key: string) => {
			await this.stores.put(storeName, {[key]: props[key]});
		});
